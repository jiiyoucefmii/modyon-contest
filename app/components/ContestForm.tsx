...
export default function ContestForm() {
  ...
  const checkExistingUser = useCallback(async (emailValue: string) => {
    ...
    try {
      const response = await fetch(`/api/giveaway/stats?email=${encodeURIComponent(emailValue)}`);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // If user already exists, just show their stats
  ...
  if (isSubmitted && user) {
    return (
      <section ref={sectionRef} className={`${styles.waitlistSection} ${isVisible ? styles.visible : ''}`}>
        <div className={styles.container}>
          <div className={styles.card}>
            <div className={styles.successState}>
              ...
              <h3>{isReturningUser ? 'Welcome Back!' : 'Welcome to the Contest!'}</h3>
              <p>
                {isReturningUser 
              ...
              <div className={styles.dashboard}>
                <div className={styles.statsGrid}>
                  ...
                  <div className={styles.statCard}>
                    <div className={styles.statNumber}>{Math.max(0, user.entries - 1)}</div>
  ...
  return (
    <section id="form" ref={sectionRef} className={`${styles.waitlistSection} ${isVisible ? styles.visible : ''}`}>
      <div className={styles.container}>
        <div className={styles.card}>
          ...
          <p className={styles.mainDescription}>
            We are changing the way people collaborate remotely<br />
          ...
          <form onSubmit={handleSubmit} className={styles.emailForm}>
            ...
            <div className={styles.referralInputContainer}>
              ...
              <div className={styles.referralHint}>
                Have a friend's referral code? Enter it above to give them credit!
              </div>
            </div>
            ...
            {isReturningUser && user && (
              <div className={styles.returningUserMessage}>
                Welcome back! You have {user.entries} entries. Click submit to view your stats.
              </div>
            )}
          ...
          <div className={styles.signupStats}>
            <div className={styles.avatarStack}>
              <div className={styles.avatarItem} style={{backgroundImage: 'url(/api/placeholder/32/32?1)'}}></div>
              <div className={styles.avatarItem} style={{backgroundImage: 'url(/api/placeholder/32/32?2)'}}></div>
              <div className={styles.avatarItem} style={{backgroundImage: 'url(/api/placeholder/32/32?3)'}}></div>
            </div>
            <span className={styles.signupText}>
              {isClient ? signupCount.toLocaleString() : signupCount}k people signed up
            </span>
          </div>
        </div>
      </div>
